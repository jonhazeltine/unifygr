...
function buildInstruction(message: string, ctx: PageContext): string {
	return [
		...
		"You MUST NOT modify: any config (astro.config.*, package.json, tsconfig, vercel.json), the editor's own code (src/lib/studio/**, src/pages/api/studio/**, src/components/StudioDock.astro), or tina/**. Do not run shell commands.",
		"",
